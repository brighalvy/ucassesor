...
forecast <- (read.csv("data/forecast.csv"))
forecast$date <- as.numeric(format(as.Date(paste("01-",forecast$Month, sep = ""), format = "%d-%y-%b"), "%Y")) + (as.numeric(format(as.Date(paste("01-",forecast$Month, sep = ""), format = "%d-%y-%b"), "%m")) - .5)/12
...
function(input, output, session){
  ...
  observeEvent(input$run, {
    updateSelectInput(session, "dist", choices = unique(final()[[1]]$new_dist))
  }
  )
  
  observeEvent(input$run, {
    updateSelectInput(session, "selectVA", choices = unique(final()[[1]]$NBHD_CODE))
  }
  )
 ...
 # })
  ...
  output$coefs <- renderTable({
    ...
    data <- cbind(names((coefs[[num]][,1])), as.matrix(coefs[[num]]))
    colnames(data) <- c("Variable", "Effect")
    data
  })
  ## Code Forecast graph
  ...
  output$ratio_hist <- renderPlot({
    ...
    ## Calculate ratio
    ratio <- ratio_data$ratio
    hist(ratio, main = "Histogram of Ratio", breaks = input$br, xlim = input$xlim, col = "darkgrey", border = "white", xlab ="Ratio" )
  })
  ...
  output$outlier_table <- renderTable(
    {
      ...
      outlier <- ratio_data %>%
        ...
        dplyr::select(SERNO, LOT_VALUE, TV_IMP_cap, TVPred, ratio)
      outlier
    }
  )
  ...
  output$pct_hist <- renderPlot(
    {
      final_df <- final()[[1]]
      hist(final_df$TVPctChange, breaks = input$brks, xlim = input$xlim2, main = "Histogram of Percent Change", col = "darkgrey", border = "white", xlab = "Percent Change")
    }
  )
  ...
  output$va_hist <- renderPlot({
    final_df <- final()[[1]]
    hist(final_df[final_df$NBHD_CODE == input$selectVA, ]$TVPctChange, breaks = 100, xlim = c(-1,2), main = "Histogram of Percent Change", col = "darkgrey", border = "white", xlab = "Percent Change")
  })
}
