...
run_model <- function(area_data, pyear) {
...
area_data <- area_data %>%
  ...
  mutate( SYEAR = as.numeric(format(as.Date(SOLD_DATE, format="%d-%h-%y"), "%Y")),
          ...
          VERYGOOD = QUALITY_DESCR == "Very Good" | QUALITY_DESCR == "Very Go",
          EXCELLENT = QUALITY_DESCR == "Excellent",
          TWOSTORY = STYLE_DESCR == '2 Story',
          ONESTORY = STYLE_DESCR == 'Ranch 1 Story',
          MULTISTORY = STYLE_DESCR == 'Bi Level 1 Story' | 
            STYLE_DESCR == 'Bi Level 2 Story' | 
            STYLE_DESCR == 'Split Level',
          OTHERSTORY = (STYLE_DESCR != '2 Story') & 
            (STYLE_DESCR != 'Ranch 1 Story') &  
            (STYLE_DESCR != 'Bi Level 1 Story') &  
            (STYLE_DESCR != 'Bi Level 2 Story') &  
            (STYLE_DESCR != 'Split Level'),
          ...
          TIME_ADJ_NET_SOLD_PRICE = SOLD_PRICE - CONCESSIONS,
          TIME_ADJ_NET_SOLD_PRICE = TIME_ADJ_NET_SOLD_PRICE * TIME_ADJ,
...
#saveRDS(sort(unique(area_data$new_dist)), "M:/shiny_trial/dist_list.rds")
...
model_mat <- model.matrix(NET_SOLD_PRICE_IMP ~ #SPLINE1 + SPLINE2 + 
                              ...
                              DET_GAR_AREA + 
                              (REMODEL_PCT) + FIN_BSMT + UNFIN_BSMT +  NO_BSMT + YrMon + YrMon:FIN_AREA , data = area_data[!is.na(area_data$SOLD_PRICE) & area_data$NET_SOLD_PRICE_IMP > 0, ])[,-c(1)]
area_data <- area_data %>% 
...
preds_mat <- model.matrix( ~  ~ #SPLINE1 + SPLINE2 + 
                               ...
                               BATH_3_4 + DELUXE_BATHS_5 + LUXURY_BATHS_6 + ATT_GAR_AREA + 
                               DET_GAR_AREA + 
                               (REMODEL_PCT) + FIN_BSMT + UNFIN_BSMT +  NO_BSMT + YrMon + YrMon:FIN_AREA, data = area_data)[,-c(1)]


  
  lambda = exp(seq(-15, 15, length = 1000))
  lasso_lm = glmnet::glmnet(model_mat, log(area_data[!is.na(area_data$SOLD_PRICE) & area_data$NET_SOLD_PRICE_IMP > 0, ]$NET_SOLD_PRICE_IMP), alpha = 1, lambda = lambda)
  #models[[i]] = rf_mod
  #plot(lasso_lm)
  lasso_cv = cv.glmnet(model_mat, log(area_data[!is.na(area_data$SOLD_PRICE) & area_data$NET_SOLD_PRICE_IMP > 0, ]$NET_SOLD_PRICE_IMP), alpha=1, lambda = lambda)
  ...
  #predsdf[[i]] <- cbind(area_data, preds)
...
#final <- bind_rows(predsdf)
final <- cbind(area_data,preds)

final$TOT_VAL2021 <- final$X2022_RE_RES + final$X2022_RE_AGR + final$X2022_RE_COM + final$X2022_IM_RES + final$X2022_IM_AGR + final$X2022_IM_COM
final$IMP_VAL2021 <-  final$X2022_IM_RES + final$X2022_IM_AGR + final$X2022_IM_COM

final <- final %>%
  dplyr::mutate(TV_IMP = exp(s1),
         TV_IMP_PCT = (TV_IMP - IMP_VAL2021) / IMP_VAL2021,
         TV_IMP_cap = ifelse(TV_IMP_PCT > .4 & YEAR_BUILT <= pyear - 2 & UPDATED == FALSE, yes = 1.4 * IMP_VAL2021, ifelse(TV_IMP_PCT < 0, yes = IMP_VAL2021, no = TV_IMP)),
         TVPred = TV_IMP_cap + LAND_VAL_RW,
         TVPctChange = (TVPred - TOT_VAL2021) / TOT_VAL2021) %>%
